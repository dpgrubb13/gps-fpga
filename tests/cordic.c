...
int main(void)
{
  uint64_t packed_data = pack_cordic(1.0, 0.0, -1.0, 0);
  reg_write64(CORDIC_WRITE, packed_data);
  uint64_t read_data = reg_read64(CORDIC_READ);
  int64_t x = unpack_cordic_x(read_data);
  int64_t y = unpack_cordic_y(read_data);
  int64_t z = unpack_cordic_z(read_data);
