...
/**
  ...
  */
abstract class CordicBlock[D, U, EO, EI, B <: Data, T <: Data : Ring : BinaryRepresentation : Order : ConvertableTo]
(
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    out.bits.data.asTypeOf(new CordicBundle(cordicParams)) := cordic.io.out.bits
    out.bits.last := false.B

//    cordic.io.in.valid := out.valid
//    out.ready := cordic.io.in.ready
//    cordic.io.in.bits := out.bits
//
//    in.valid := cordic.io.out.valid
//    cordic.io.out.ready := in.ready
//    in.bits := cordic.io.out.bits
  }
}
...
/**
  ...
  */
class TLCordicBlock[T <: Data : Ring : BinaryRepresentation : Order : ConvertableTo]
(
...
/**
  ...
  * @tparam T Type parameter for cordic, i.e. FixedPoint or DspReal
  */
class CordicThing[T <: Data: Ring : BinaryRepresentation : Order : ConvertableTo]
(
  val cordicParams: CordicParams[T],
  val depth: Int = 8,
